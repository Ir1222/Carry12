...
def test_nforce_csv_roundtrip_recomputes_summary_and_appends():
    rows = [sample_velocity_tracking(_FakeEnv(), time_s=3.0, policy_step=150)]
    with tempfile.TemporaryDirectory() as directory:
        logger = NForceVelocityCsvLogger(directory)
        trace_path = logger.write_trace("T0001", rows)
        logger.append_summary(_summary(rows))
        logger.append_summary(_summary(rows))
        with open(trace_path, newline="") as file:
            trace = list(csv.DictReader(file))
        with open(logger.summary_path, newline="") as file:
            reader = csv.DictReader(file)
            assert tuple(reader.fieldnames) == SUMMARY_FIELDS
            summaries = list(reader)
        assert len(summaries) == 2
        numeric_trace = [{key: float(value) if key != "phase" else value
                          for key, value in row.items()} for row in trace]
        recomputed = _summary(numeric_trace)
        for key in ("vx_mae", "vy_rmse", "yaw_rate_abs_error_p95",
                    "lin_vel_error_norm_mean", "steady_carry_duration_s"):
...
def test_nforce_csv_rejects_existing_different_header():
    with tempfile.TemporaryDirectory() as directory:
        logger = NForceVelocityCsvLogger(directory)
        Path(logger.summary_path).write_text("old,header\n1,2\n")
        with TestCase().assertRaisesRegex(ValueError, "schema does not match"):
            logger.append_summary(_summary([]))
        assert Path(logger.summary_path).read_text() == "old,header\n1,2\n"
...
if __name__ == "__main__":
    tests = (
        ...
        test_nforce_csv_roundtrip_recomputes_summary_and_appends,
        test_nforce_csv_rejects_existing_different_header,
    )
    for test in tests:
        test()
