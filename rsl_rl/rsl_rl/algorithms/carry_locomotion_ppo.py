...
from rsl_rl.storage import CarryLocomotionRolloutStorage


class CarryLocomotionPPO:
    ...
    def __init__(
        ...
        self.transition = CarryLocomotionRolloutStorage.Transition()
    ...
    def init_storage(
        ...
        self.storage = CarryLocomotionRolloutStorage(
            num_envs,
            num_transitions_per_env,
            actor_obs_shape,
            critic_obs_shape,
    ...
    def act(self, obs, critic_obs):
        if obs.isnan().any():
            obs = torch.zeros(
                (obs.shape[0], obs.shape[1]), device=obs.device
            )
            critic_obs = torch.zeros(
                (critic_obs.shape[0], critic_obs.shape[1]), device=obs.device
            )
    ...
    def update(self):
        ...
        mean_policy_smooth_loss = 0
        mean_value_smooth_loss = 0
        ...
        for (
            ...
            value_smooth_coef = (
                self.value_smoothness_coef * policy_smooth_coef
            )

            mix_weights = (
                cont_batch * (torch.rand_like(cont_batch) - 0.5) * 2.0
            )
            mix_obs_batch = obs_batch + mix_weights * (
                next_obs_batch - obs_batch
            )
            mix_critic_obs_batch = critic_obs_batch + mix_weights * (
                next_critic_obs_batch - critic_obs_batch
            )

            policy_smooth_loss = torch.square(
                torch.norm(
                    mu_batch
                    - self.actor_critic.act_inference(mix_obs_batch),
                    dim=-1,
                )
            ).mean()
            value_smooth_loss = torch.square(
                torch.norm(
                    value_batch
                    - self.actor_critic.evaluate(mix_critic_obs_batch),
                    dim=-1,
                )
            ).mean()
            smooth_loss = (
            ...
            mean_policy_smooth_loss += policy_smooth_loss.item()
            mean_value_smooth_loss += value_smooth_loss.item()

        num_updates = self.num_learning_epochs * self.num_mini_batches
        mean_value_loss /= num_updates
        mean_surrogate_loss /= num_updates
        mean_policy_smooth_loss /= num_updates
        mean_value_smooth_loss /= num_updates
        self.storage.clear()
